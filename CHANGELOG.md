...
- “控制在线车机”升级为专属底部控制面板：56dp 标题栏、16dp 卡片圆角、56dp 播放/暂停主按钮、48dp 切歌按钮、52dp 投送歌曲主操作。
- 支持将一台在线车机设为默认设备；多台车机按默认设备优先展示。
- 单台车机的控制请求不会冻结其他卡片；刷新请求会在切歌或投送歌曲后失效，避免旧状态覆盖刚刚完成的操作结果。
- 增加“更新于”状态、空状态重试入口及明确的操作回执。
